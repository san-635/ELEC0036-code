...
# goal: create ehr_cxr_dataset_comp/train and ehr_cxr_dataset_comp/test directories with the following files:
#       ehr_cxr_dataset_comp/{partition}/{subject_id}_{stay_id}_timeseries.csv (one per episode)
#       ehr_cxr_dataset_comp/{partition}/listfile.csv (one per partition)
    ...
    for _, row in tqdm(partition_df.iterrows(), total=partition_df.shape[0], desc=f'Iterating over episodes in ehr_root/{partition}'):
        ...
        for ts_filename in episode_ts_files:
            with open(os.path.join(episode_dir, ts_filename)) as tsfile:
                ...
                # 3/6: write the modified timeseries file into a new csv file at the corresponding ehr_cxr_dataset_comp/{partition}
                ...
                # 6/6: move this episode's linked CXR image to ehr_cxr_dataset/CXR_images and get its file path
...
def main():
    parser = argparse.ArgumentParser(description="16_create_ehr_cxr_comp_dataset.py")
    parser.add_argument('episode_root_path', type=str, help="'ehr_root' directory containing EHR train, val and test sets.")
    parser.add_argument('output_path', type=str, help="'ehr_cxr_dataset_comp' directory where the created EHR+CXR dataset should be written.")
    ...
    # save samples_{partition} as a single non-timeseries file, listfile.csv file, at ehr_cxr_dataset_comp/{partition}
