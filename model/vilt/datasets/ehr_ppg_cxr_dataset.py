...
class EHRPPGCXRDataset_win(Dataset):
    def __init__(self, discretizer, normalizer, ppg_normalizer, split, dataset_dir, max_text_len, max_ppg_len, ppg_overlap, transform_keys, image_size):
        self.discretizer = discretizer
        self.normalizer = normalizer
        self.ppg_normalizer = ppg_normalizer
        assert split in ["train", "val", "test"]
        self.split = split
        self._dataset_dir = dataset_dir
        self.max_text_len = max_text_len    # max_text_len - 1 is passed since [class] token is prepended later
        self.max_ppg_len = max_ppg_len      # max_ppg_len - 1 is passed since [class] token is prepended later
        self.ppg_overlap = ppg_overlap
        ...
        self._data = [re.sub(r'(\[.*?\])', lambda x: x.group(1).replace(',', '|'), line) for line in self._data]  # replace commas in PPG signal list with '|' to avoid splitting the signal
        ...
        self.data_map = {
            mas[0]: {
                ...
                'ppg': np.array([float(i.strip("'")) for i in mas[7].strip('[]').split('| ')]).reshape(1, -1),  # convert string "['val1'| 'val2'| ...| 'val1250']" to 2D np array [[val1, val2, ..., val1250]]
                'img_path': mas[8].split('\n')[0]
    ...
    def get_text(self, index):
        ...
        data, t_mask = self.discretizer.transform(ts_data, non_ts_data, max_rows=self.max_text_len)
        data = self.normalizer.transform(data)                         
        
        # windowing (w/o overlap)
        ppg_signal = self.ppg_normalizer.transform(ret["ppg"])
        ppg_data = ppg_signal.reshape(self.max_ppg_len, -1)              # shape: (max_ppg_len-1, 1250/(max_ppg_len-1))
        ppg_data = torch.tensor(ppg_data, dtype=torch.float32)
        
        p_mask = torch.ones(ppg_data.shape[0], dtype=torch.float32)      # len: (max_ppg_len-1)

        label = int(ret["label"])
        img_path = ret["img_path"]
        return torch.tensor(data, dtype=torch.float32), torch.tensor(t_mask, dtype=torch.float32), ppg_data, p_mask, label, img_path
    ...
    def __getitem__(self, index):
        data, t_mask, ppg_data, p_mask, label, img_path = self.get_text(index)
        ...
        return {
            ...
            "t_mask": t_mask,       # a 1D tensor (dim: no. of bins)
            "ppg": ppg_data,        # a 2D tensor (dim: window size, 1250/window size) or (dim: window size, new_dim)
            "p_mask": p_mask,       # a 1D tensor (dim: window size)
    ...
    def my_collate(self, batch):                                                    # batch = [{'image': 3D tensor, ..., 'label': scalar}, ..., {'image': 3D tensor, ..., 'label': scalar}]
        ...
        keys = set([key for sample in batch for key in sample.keys()])              # ('image', 'text', 't_mask', 'ppg', 'p_mask', 'label')
        dict_batch = {key: [sample[key] for sample in batch] for key in keys}       # {'image': [list of 3D tensors], 'text': [list of 2D tensors], 't_mask': [list of 1D tensors], 'ppg': [list of 2D tensors], 'p_mask': [list of 1D tensors], 'label': [list of scalars]}
        ...
        mask_np = [np.array(m) for m in t_mask]     # convert to a list of 1D np arrays for padding operation
        ...
        dict_batch["t_mask"] = mask_tensor          # dict_batch["t_mask"] = 2D tensor with shape: (batch_size, max_text_len-1)

        # ADDITIONAL MODALITY - NO PADDING REQUIRED FOR PPG SIGNAL OR ITS MASK #
        ppg_tensor = torch.stack(dict_batch["ppg"])
        dict_batch["ppg"] = ppg_tensor        # dict_batch["ppg"] = 3D tensor of shape: (batch_size, max_ppg_len-1, 1250/(max_ppg_len-1))

        p_mask_tensor = torch.stack(dict_batch["p_mask"])
        dict_batch["p_mask"] = p_mask_tensor  # dict_batch["p_mask"] = 2D tensor of shape: (batch_size, max_ppg_len-1)
    ...
    def pad_zeros_mask(self, t_mask):
        dtype = t_mask[0].dtype
        # concatenate each t_mask 1D array with a zero-filled array (of appropriate size) such that padded_mask.shape[0] = max_text_len-1
