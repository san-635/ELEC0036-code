...
from vilt.datasets import EHRPPGCXRDataset_win, EHRPPGCXRDataset_win_cv
from vilt.ehr_utils.utils import Discretizer, Normalizer, Normalizer2

class EHRPPGCXRDataModule_win(LightningDataModule):  # BaseDataModule is a PyTorch Lightning's LightningDataModule subclass
    def __init__(self, _config):
        super().__init__()

        self.data_dir = _config["data_root"]                # ./ehr_ppg_cxr_dataset; path to the dataset relative to run.py
        ...
        self.max_ppg_len = _config["max_ppg_len"]-1         # self.max_ppg_len is -1 since [class] token is prepended later
        self.ppg_overlap = _config["ppg_overlap"]
    ...
    def set_train_dataset(self):
        ...
        ppg_normalizer_state = 'ppg_normalizer__{}_{}h_{}'.format(self.max_text_len, self.timestep, self.impute_strategy)
        ppg_normalizer_path = os.path.join(os.path.dirname(__file__), "..", "ehr_utils", ppg_normalizer_state)
        ppg_normalizer.load_params(ppg_normalizer_path)
        ...
        self.train_dataset = self.dataset_cls(
            ...
            max_ppg_len=self.max_ppg_len,
            ppg_overlap=self.ppg_overlap,
