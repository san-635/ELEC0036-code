...
class Attention(nn.Module):
    def __init__(
        ...
        self.scale = qk_scale or head_dim ** -0.5       # 1 / sqrt(64) = 1/8 or 1/ sqrt(qk_scale) if qk_scale is set
    ...
    def forward(self, x, mask=None):
        # compute queries (q), keys (k), and values (v) using linear projection of x
        B, N, C = x.shape                   # B: batch_size = 6, N: seq length = max_text_len + image_len + max_ppg_len, C: channels
        # print("--> x.shape: ", B, ",", N, ",", C)
        qkv = (
            self.qkv(x)                                             # nn.Linear; returns linear projection of x of shape: (B, N, 3 * C)
            .reshape(B, N, 3, self.num_heads, C // self.num_heads)  # reshapes to (B, N, 3, 12, C // 12)
            .permute(2, 0, 3, 1, 4)                                 # final shape of qkv: (3, B, 12, N, C // 12)
        )
        # print("--> qkv.shape: ", qkv.shape)
        q, k, v = (qkv[0], qkv[1], qkv[2])  # make torchscript happy (cannot use tensor as tuple); each of shape: (B, 12, N, C // 12)

        # scaled dot-product of q and k
        attn = (q @ k.transpose(-2, -1)) * self.scale   # shape: (B, 12, N, N)
        ...
        if mask is not None:
            ...
            attn = attn.masked_fill(mask_inv, float("-inf")) # fill attn with -inf where mask is False; shape: (B, 12, N, N)

        # compute final attn weights using softmax
        attn = attn.softmax(dim=-1)
        attn = self.attn_drop(attn)     # nn.Dropout; apply dropout

        # compute weighted sum of values (v) using attn weights
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        x = self.proj(x)                # nn.Linear; shape: (B, N, C)
        x = self.proj_drop(x)           # nn.Dropout; apply dropout
...
class VisionTransformer(nn.Module):
    ...
    def visual_embed(self, _x, max_image_len=200, mask_it=False):
        ...
        select = select.to(patch_index.device)  ### added
...
@register_model
def vit_base_patch32_384(pretrained=False, **kwargs):   # this is the model we use
