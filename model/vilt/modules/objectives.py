...
# used for image text matching with word patch alignment task
def trace(x):
...
# loss for the masked language modeling task
...
# loss for the masked patch prediction task
...
# loss for the image text matching with word patch alignment task
...
# loss for EHR+CXR dataset task     ### added
def compute_ehr_cxr(pl_module, batch):
    phase = "train" if pl_module.training else "val"
    # call infer function from vilt_module.py, returns ret dict with cls_feats, etc.
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    else:
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)

    # call ehr_cxr_classifier function from vilt_module.py, returns classification output (i.e., logits)
    imgcls_logits = pl_module.ehr_cxr_classifier(infer["cls_feats"])
    imgcls_labels = batch["label"]
    imgcls_labels = torch.tensor(imgcls_labels).to(pl_module.device).long()
    imgcls_loss = F.cross_entropy(imgcls_logits, imgcls_labels) # compute cross entropy loss since binary classification

    ret = {"ehr_cxr_loss": imgcls_loss, "ehr_cxr_logits": imgcls_logits, "ehr_cxr_labels": imgcls_labels}

    # getattr returns a Metric object, which is then called with the arguments in the second set of parentheses
    # update the Metric object with this batch's loss, accuracy and AUROC, which are used later in epoch_wrapup via .compute()
    loss = getattr(pl_module, f"{phase}_ehr_cxr_loss")(ret["ehr_cxr_loss"])
    acc = getattr(pl_module, f"{phase}_ehr_cxr_accuracy")(ret["ehr_cxr_logits"], ret["ehr_cxr_labels"])
    auroc = getattr(pl_module, f"{phase}_ehr_cxr_AUROC")(ret["ehr_cxr_logits"], ret["ehr_cxr_labels"])
    
    pl_module.log(f"ehr_cxr/{phase}/loss", loss)

    return ret

# loss for EHR+PPG+CXR dataset task     ### added
def compute_ehr_ppg_cxr(pl_module, batch):
    phase = "train" if pl_module.training else "val"
    # call infer function from vilt_module.py, returns ret dict with text_feats, image_feats, cls_feats, etc.
    if phase == "train":
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)
    else:
        infer = pl_module.infer(batch, mask_text=False, mask_image=False)

    # call ehr_ppg_cxr_classifier function from vilt_module.py, returns classification output (i.e., logits)
    imgcls_logits = pl_module.ehr_ppg_cxr_classifier(infer["cls_feats"])
    imgcls_labels = batch["label"]
    imgcls_labels = torch.tensor(imgcls_labels).to(pl_module.device).long()
    imgcls_loss = F.cross_entropy(imgcls_logits, imgcls_labels) # compute cross entropy loss since binary classification

    ret = {"ehr_ppg_cxr_loss": imgcls_loss, "ehr_ppg_cxr_logits": imgcls_logits, "ehr_ppg_cxr_labels": imgcls_labels}

    # getattr returns a Metric object, which is then called with the arguments in the second set of parentheses
    # update the Metric object with this batch's loss, accuracy and AUROC, which are used later in epoch_wrapup via .compute()
    loss = getattr(pl_module, f"{phase}_ehr_ppg_cxr_loss")(ret["ehr_ppg_cxr_loss"])
    acc = getattr(pl_module, f"{phase}_ehr_ppg_cxr_accuracy")(ret["ehr_ppg_cxr_logits"], ret["ehr_ppg_cxr_labels"])
    auroc = getattr(pl_module, f"{phase}_ehr_ppg_cxr_AUROC")(ret["ehr_ppg_cxr_logits"], ret["ehr_ppg_cxr_labels"])
    
    pl_module.log(f"ehr_ppg_cxr/{phase}/loss", loss)

    return ret

# loss for the Hateful Memes task
def compute_hatememes(pl_module, batch):
    ...
    imgcls_labels = batch["label"]
    imgcls_labels = torch.tensor(imgcls_labels).to(pl_module.device).long()
    imgcls_loss = F.cross_entropy(imgcls_logits, imgcls_labels) # compute cross entropy loss for binary classification
...
# loss for the UPMC Food-101 task
...
# loss for Visual Question Answering (VQAv2) task
...
# loss for the Natural Language Visual Reasoning (NLVR2) task
...
# loss for Image Retrieval (IR) and Text Retrieval (Flickr30K and MSCOCO) task
...
# initialise weights for different modules of transformer
...
# test step function for the VQAv2 task specifically
...
# test epoch wrapup function for the VQAv2 task specifically
