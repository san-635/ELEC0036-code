...
def epoch_wrapup(pl_module):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        elif loss_name == "ehr_cxr":    ### added
            value2 = getattr(pl_module, f"{phase}_{loss_name}_accuracy").compute()  # compute average of accuracies of all batches in this epoch
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value2)       
            getattr(pl_module, f"{phase}_{loss_name}_accuracy").reset()

            value = getattr(pl_module, f"{phase}_{loss_name}_AUROC").compute()      # compute average of AUROCs of all batches in this epoch
            pl_module.log(f"{loss_name}/{phase}/AUROC_epoch", value)            
            getattr(pl_module, f"{phase}_{loss_name}_AUROC").reset()
            
            loss_value = getattr(pl_module, f"{phase}_{loss_name}_loss").compute()  # compute average of losses of all batches in this epoch
        ...
        elif loss_name == "ehr_ppg_cxr":    ### added
            value2 = getattr(pl_module, f"{phase}_{loss_name}_accuracy").compute()  # compute average of accuracies of all batches in this epoch
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", value2)       
            getattr(pl_module, f"{phase}_{loss_name}_accuracy").reset()
    ...
    pl_module.log(f"{phase}/the_metric", the_metric)    # same as AUROC_epoch for loss names ehr_cxr and ehr_ppg_cxr
...
def set_schedule(pl_module):
    ...
    head_names = ["vqa_classifier", "ehr_cxr_classifier", "ehr_ppg_cxr_classifier", "mmimdb_classifier", "food101_classifier", "hatememes_classifier", "nlvr2_classifier"]  ### added
    prompt_name = "prompt"
    lr_mult = pl_module.hparams.config["lr_mult"]           # 1
    end_lr = pl_module.hparams.config["end_lr"]             # 0
    decay_power = pl_module.hparams.config["decay_power"]   # 1
    optim_type = pl_module.hparams.config["optim_type"]     # "adamw"
    ...
    if pl_module.trainer.max_steps is None: # True in our case
        max_steps = (
            len(pl_module.trainer.datamodule.train_dataloader())    # number of batches in training set
            * pl_module.trainer.max_epochs                          # config["max_epoch"]
            // pl_module.trainer.accumulate_grad_batches            # always 1 in our case
        )   # max_steps is num of batches in training set * config["max_epoch"]
    else:
        max_steps = pl_module.trainer.max_steps

    warmup_steps = pl_module.hparams.config["warmup_steps"]
    if isinstance(pl_module.hparams.config["warmup_steps"], float):
        warmup_steps = int(max_steps * warmup_steps)    ## warmup_steps = int(0.1 * num of batches in training set * config["max_epoch"])
    ...
    else:   # decay_power == 1 in our case
