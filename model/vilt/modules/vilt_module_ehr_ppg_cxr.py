...
class ViLTransformerSS_win(pl.LightningModule):
    def __init__(self, config):
        ...
        ### added
        # ppg embedding module
        self.ppg_embeddings = heads.PPGEmbedding_p(config["hidden_size"])   # use PPGEmbedding_p irrespective of with or w/o overlap
        self.ppg_embeddings.apply(objectives.init_weights)
        ...
        # task-specific classifiers depending on the datasets
        ...
        if self.hparams.config["loss_names"]["ehr_cxr"] > 0:    # for EHR+CXR dataset (binary classification)
        ...
        if self.hparams.config["loss_names"]["ehr_ppg_cxr"] > 0:    # for EHR+PPG+CXR dataset (binary classification)     ### added
            cls_num = self.hparams.config["class_num"]  # 2
            self.ehr_ppg_cxr_classifier = nn.Sequential(
                nn.Linear(hs, hs * 2),
                nn.LayerNorm(hs * 2),
                nn.GELU(),
                nn.Linear(hs * 2, cls_num),
            )
            self.ehr_ppg_cxr_classifier.apply(objectives.init_weights)
    ...
    def infer(
        ...
        if image_embeds is None and image_masks is None:
            ...
            (
                ...
                img,
                max_image_len=self.hparams.config["max_image_len"],     # -1
                mask_it=mask_image,
            )
        else:
        ...
        text_embeds = self.text_embeddings(batch["text"])                               # shape: batch_size, max_text_len-1, ehr_n_var -> batch_size, max_text_len, hidden_size
        
        # ppg embeddings - pass PPG data through PPGEmbedding module to get ppg_embeds     ### added
        ppg_embeds = self.ppg_embeddings(batch["ppg"])                                  # shape: batch_size, max_ppg_len-1, 1250/(max_ppg_len-1) -> batch_size, max_ppg_len, hidden_size
        
        # add modal-type embeddings (tensors of 0s for text, 1s for image, 2s for PPG) to image, text and PPG embeddings; shapes are unchanged
        text_embeds, image_embeds, ppg_embeds = (
            ...
            ppg_embeds + self.token_type_embeddings(torch.full_like(ppg_embeds[:, :, 0], 2).long())            ### added
        )

        # prepend cls_token to masks of EHR and PPG data
        text_masks = batch["t_mask"]                                                    # shape: batch_size, max_text_len-1
        text_cls_token = torch.zeros((text_masks.size(0), 1), device=text_masks.device)
        text_masks = torch.cat([text_cls_token, text_masks], dim=1)                     # shape: batch_size, max_text_len

        ### added
        ppg_masks = batch["p_mask"]                                                     # shape: batch_size, max_ppg_len-1
        ppg_cls_token = torch.zeros((text_masks.size(0), 1), device=ppg_masks.device)
        ppg_masks = torch.cat([ppg_cls_token, ppg_masks], dim=1)                        # shape: batch_size, max_ppg_len

        # concatenate all embeddings, and all masks (by rows of each sample)
        co_embeds = torch.cat([text_embeds, image_embeds, ppg_embeds], dim=1)           # shape: batch_size, max_text_len + image_embeds.size(1) + max_ppg_len, hidden_size     ### modified
        co_masks = torch.cat([text_masks, image_masks, ppg_masks], dim=1)               # shape: batch_size, max_text_len + image_masks.size(1) + max_ppg_len            ### modified
    ...
    def forward(self, batch):
        ...
        # Binary classification for EHR+CXR dataset
        if "ehr_cxr" in self.current_tasks:
            ret.update(objectives.compute_ehr_cxr(self, batch))
            
        # Binary classification for EHR+PPG+CXR dataset    ### added
        if "ehr_ppg_cxr" in self.current_tasks:
            ret.update(objectives.compute_ehr_ppg_cxr(self, batch))
    ...
    def test_step(self, batch, batch_idx):
        vilt_utils.set_task(self)
        output = self(batch)            # call forward() with batch as input argument
        ret = dict()                    # but return an empty dict since it's a test step
        return ret
